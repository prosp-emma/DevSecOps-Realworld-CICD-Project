...
pipeline{
    ...
    stages {
        ...
        stage('Checkout from Git'){
            steps{
                git branch: 'dev-sec-ops-cicd-pipeline-project-one', url: 'https://github.com/prosp-emma/DevSecOps-Realworld-CICD-Project'
            }
        }
            ...
            steps{
                script{
                   withDockerRegistry(credentialsId: 'DockerHub-Credential', toolName: 'Docker'){
                       ...
                       sh "docker tag reddit emma64/reddit:latest "
                       sh "docker push emma64/reddit:latest "
                    }
                }
            }
        }
        stage("Trivy App Image Scan"){
            steps{
                sh "trivy image emma64/reddit:latest > trivy_image_analysis_report.txt"
            }
        }
        ...
        stage('ZAP Dynamic Testing | DAST') {
            steps {
                sshagent(['OWASP-Zap-Credential']) {
                    sh 'ssh -o StrictHostKeyChecking=no ubuntu@18.119.102.44/ "docker run -t owasp/zap2docker-stable zap-baseline.py -t http://3.144.219.101:30000/" || true'
    ...
    always {
        ...
        slackSend channel: 'emma-jenkins-cicd-pipeline-alerts', //update and provide your channel name
